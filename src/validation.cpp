...
#if defined(NDEBUG)
# error "Ideology cannot be compiled without assertions."
#endif
...
std::unique_ptr<IdeologyState> globalState;
...
const std::string strMessageMagic = "Ideology Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = ideoDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            IdeologyTxConverter converter(tx, NULL);
            ExtractIdeologyTX resultConverter;
            if(!converter.extractionIdeologyTransactions(resultConverter)){
            ...
            std::vector<IdeologyTransaction> ideoTransactions = resultConverter.first;
            std::vector<EthTransactionParams> ideoETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(IdeologyTransaction ideoTransaction : ideoTransactions){
                sumGas += ideoTransaction.gas() * ideoTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, ideoTransaction.gasPrice());
                } else {
                    txMinGasPrice = ideoTransaction.gasPrice();
                }
                VersionVM v = ideoTransaction.getVersion();
                if(v.format!=0)
                ...
                if(ideoTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(ideoTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(ideoTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)ideoTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(ideoETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > ideoTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s IDEO additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // ideo
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // ideo
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    IdeologyTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<IdeologyTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path ideoDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(ideoDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(ideoDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(IdeologyTransaction& tx : txs){
        //validate VM version
...
bool IdeologyTxConverter::extractionIdeologyTransactions(ExtractIdeologyTX& ideotx){
    std::vector<IdeologyTransaction> resultTX;
    ...
    ideotx = std::make_pair(resultTX, resultETP);
    return true;
}

bool IdeologyTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool IdeologyTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
IdeologyTransaction IdeologyTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    IdeologyTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = IdeologyTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = IdeologyTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // ideo
    IdeologyDGP ideoDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setIdeologySchedule(ideoDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = ideoDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = ideoDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = ideoDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ideo
...
///////////////////////////////////////////////////////////////////////////////////////// ideo
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            IdeologyTxConverter convert(tx, &view, &block.vtx);

            ExtractIdeologyTX resultConvertIdeologyTX;
            if(!convert.extractionIdeologyTransactions(resultConvertIdeologyTX)){
            ...
            if(!CheckMinGasPrice(resultConvertIdeologyTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertIdeologyTX.first, blockGasLimit, pindex->pprev);
            ...
            for(IdeologyTransaction& qtx : resultConvertIdeologyTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertIdeologyTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertIdeologyTX.first[k].from(), resultConvertIdeologyTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // ideo
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // ideo
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // ideo
            globalState->setRootUTXO(oldHashUTXORoot); // ideo
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // ideo
        uint32_t sizeBlockDGP = ideoDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Ideology PoS we need to sync this after every block to ensure txdb is populated for
