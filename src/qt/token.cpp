...
bool ToHash160(const std::string& strIdeologyAddress, std::string& strHash160)
{
    CTxDestination ideoAddress = DecodeDestination(strIdeologyAddress);
    if(!IsValidDestination(ideoAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&ideoAddress);
    if(keyid){
...
bool ToIdeologyAddress(const std::string& strHash160, std::string& strIdeologyAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination ideoAddress = keyid;
    if(IsValidDestination(ideoAddress)){
        strIdeologyAddress = EncodeDestination(ideoAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToIdeologyAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToIdeologyAddress(tokenEvent.receiver, tokenEvent.receiver);
