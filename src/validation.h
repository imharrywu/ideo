...
using ExtractIdeologyTX = std::pair<std::vector<IdeologyTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // ideo
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashIdeologyRoot,
    const dev::h256& oldHashStateRoot, const std::vector<IdeologyTransaction>& transactions);
...
class IdeologyTxConverter{

public:

    IdeologyTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionIdeologyTransactions(ExtractIdeologyTX& ideoTx);
...
private:
    ...
    IdeologyTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<IdeologyTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
