#include <ideo/ideoDGP.h>

void IdeologyDGP::initDataEIP158(){
...
bool IdeologyDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule IdeologyDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t IdeologyDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t IdeologyDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t IdeologyDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t IdeologyDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool IdeologyDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void IdeologyDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void IdeologyDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void IdeologyDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void IdeologyDGP::createParamsInstance(){
...
dev::Address IdeologyDGP::getAddressForBlock(unsigned int blockHeight){
...
void IdeologyDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void IdeologyDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void IdeologyDGP::parseStorageOneUint64(uint64_t& value){
...
void IdeologyDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule IdeologyDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
