...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setIdeologySchedule(ideoDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = ideoDGP.getBlockSize(nHeight);
    minGasPrice = ideoDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = ideoDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    IdeologyTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractIdeologyTX resultConverter;
    if(!convert.extractionIdeologyTransactions(resultConverter)){
    ...
    for(IdeologyTransaction ideoTransaction : ideoTransactions){
        txGas += ideoTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + ideoTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(ideoTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, ideoTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "ideostake";
