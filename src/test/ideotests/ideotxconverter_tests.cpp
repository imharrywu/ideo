...
void checkResult(bool isCreation, std::vector<IdeologyTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    IdeologyTxConverter converter(transaction, NULL);
    ExtractIdeologyTX ideoTx;
    BOOST_CHECK(converter.extractionIdeologyTransactions(ideoTx));
    std::vector<IdeologyTransaction> result = ideoTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ideotxconverter_tests, TestingSetup)
