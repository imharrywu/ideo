...
*Setup instructions for a Gitian build of Ideology Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ideology
...
- [Building Ideology Core](#building-ideo-core)
...
Clone the git repositories for ideo and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ideoproject/ideo --recursive
git clone https://github.com/ideoproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/ideo/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the ideo repository under 'Fetch and create inputs' to install sources which require
...
To build Ideology Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ideo repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ideo/.git/
...
For example:
```bash
URL=https://github.com/laanwj/ideo.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ideo=${COMMIT} --url ideo=${URL} ../ideo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ideo=${COMMIT} --url ideo=${URL} ../ideo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ideo=${COMMIT} --url ideo=${URL} ../ideo/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-ideo:
./bin/gbuild --commit ideo=${COMMIT},cpp-eth-ideo=${ETHCOMMIT} --url ideo=${URL},cpp-eth-ideo=${ETHURL} ../ideo/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the ideo git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ideo/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ideoproject/ideo-detached-sigs.git

BTCPATH=/some/root/path/ideo
SIGPATH=/some/root/path/ideo-detached-sigs

./bin/gbuild --url ideo=${BTCPATH},signature=${SIGPATH} ../ideo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ideo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ideo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ideo-osx-build.assert
```
...
[ideoproject/gitian.sigs](https://github.com/ideoproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@ideo.org) and he will commit them.